 ...
 * ## このツールを掛け続けるのは治療ではない
 *
 * 2 回目の `normalize:keys` で確かめた（実測 2026-08-18、`docs/normalization-recurrence.md`）。
 *
 * **前回外した 8 件のうち 7 件が再発した。**
 * 再発しなかった 1 件だけが、辞書側の書き方も直したものだった。
 *
 * > **消すのは対処であり、直すのは修理である。**
 *
 * 検出器としては正しく働く。しかし再発した 7 件が要求しているのは
 * 「似た用語を素人語のレベルで区別できるように書き直す」ことであり、
 * 出力を消して回ることではない。
 *
 * 使い方:
