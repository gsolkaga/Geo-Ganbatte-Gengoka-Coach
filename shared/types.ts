...
export interface Term {
    ...
    countries: string[]
    /**
     * **これが見えたら候補から外れる国。** `countries` とは別の仕組みである。
     *
     * ## 網羅でなくても使える
     *
     * `countries`（積集合）は**網羅でなければ使えない。**
     * 「ユーカリがある国」の全部を書けないので、ユーカリでは絞れない。
     *
     * 除外は逆で、**1 カ国について「そこには無い」と分かれば使える。**
     *
     * ```
     * ß を見た → スイスは消える（スイスは ss と書く）
     *   → ß を使う国の全リストは要らない
     * Google 車のアンテナが見えた → 北マケドニア・セルビアは消える
     *   → アンテナがある国の全リストは要らない
     * ```
     *
     * `docs/offline-works/geo_guessr_reasoning_system.md` §4-6 の
     ...
     * ## 積集合より後に効かせる
     *
     * 積集合を作ったあとに引く。**引き算は網羅を要求しない**ため、
     * `exhaustive: false` の用語でも `excludes` は使える。
     */
    excludes?: string[]
    /**
     ...
     * 設計書 §7 にこうある。
     *
     * > フィリピンのトゥクトゥクの屋根は北部から南部にかけて高さに差がある
     *
     * 連続変化するものを二値の弁別子として扱うと、**中間の地点で必ず外す。**
     * この欄がある用語は、**「見えた／見えない」で候補を切ってはならない。**
     *
     * リージョンゲスは対象外なので、これは**説明の質のためにある。**
     * 「北へ行くほど背が高くなる」と言えることが、
     * 「背が高いから北部だ」と断定することより学習に効く。
     */
    gradient?: { axis: 'north_south' | 'west_east' | 'urban_rural' | 'highland_lowland' | 'cold_warm', note: string }
    ...
    disputed: boolean
    /**
     * `countries` が**網羅**なのか**連想**なのか。既定は網羅（`true`）。
     *
     * ## 連想を積集合に入れると、連想が主張になる
     *
     * 人手ワークシート §9 にこうある。
     *
     * ```
     ...
     * ```
     *
     * これは**「次にどこを考えるか」の記録**であり、
     * 「木が道路の近くにある国はこの 3 つである」という主張ではない。
     *
     * それを積集合に入れたところ、オーストラリアの出題で
     * **積集合が 1 カ国（インドネシア）になった**（実測 2026-08-17）。
     * 数字の上では「1 カ国まで絞れた」が、**正解ではない。**
     *
     * > **1 カ国に絞れたことは、正解が分かったことではない。**
     *
     * `source` で `human` と `reference` を分けたが、
     * **絞り込み計算は両方を同じに扱っていた。** 由来を分けても、
     * 使い方を分けなければ意味がない。
     *
     * ## どちらに使うか
     *
     ...
     * |---|---|
     * | 積集合・絞り込み力の計算 | **使わない** |
     * | 素人語 → 用語の対応づけ | 使う（言葉を教えるのは有効） |
     * | 「次にどこを考えるか」の提示 | 使う（それが元の目的である） |
     *
