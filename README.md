...
並びは**地理順**にしてある（北欧 → バルト → 西欧 →……→ 南米 → オセアニア）。
人は「北欧のどこか」と絞ってから国を見比べるので、
五十音順に並べると隣り合うべき国が画面の端と端に分かれる。
...
画面は学習画面と同じ形にしてある。左に風景（4:3）、右に 14 スロット。
...
app/pages/                            /（学習）/admin（正解タグ）/datasets（配布）
