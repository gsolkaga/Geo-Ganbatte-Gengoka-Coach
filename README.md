...
```
[ 左 7 : 風景（Street View。16:9 固定）  ] [ 右 3 : 観察メモ 14 の欄 ]
[   下から「回答」の板がスライドして重なる   ] [   ここは最後まで見える  ]
------------------------------------------------------------------
[ 全幅 : コードで確定した判定 → AI の解釈（採点後に下へ積む）        ]
```

### 風景の縦横比は 16:9 に固定してある
...
```
[回答を開く]  候補: 日本（high）　決め手: script　v1 / 1 リクエスト
```
